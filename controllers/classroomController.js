...
exports.getAllClassrooms = async (req, res) => {
  try {
    ...
    const classroomsWithCount = await Promise.all(classrooms.map(async (c) => {
      let extraDetails = {};
      
      // Populate details for Home Courses with registered students
      if (c.type === 'home' && c.students && c.students.length > 0) {
         try {
           const student = await User.findById(c.students[0]).select('homeCourseDetails name');
           if (student) {
             // Add student name as title/subtitle if needed, but let's stick to details
             if (student.homeCourseDetails && c.course) {
               const details = student.homeCourseDetails.find(d => d.course && d.course.toString() === c.course._id.toString());
                if (details) {
                   extraDetails = {
                     address: details.address,
                     phoneNumber: details.phoneNumber,
                     ageRange: details.ageRange,
                     studentCount: details.studentCount, // specific count from details
                     preferredSchedule: details.preferredSchedule
                   };
                } 
             }
          }
         } catch (e) {
           console.error('Error fetching student details for classroom:', c._id, e);
         }
      }
      ...
      return {
        ...
        studentCount: (c.students && c.students.length > 0) 
            ? (extraDetails.studentCount || c.students.length) 
            : (c.studentCount || 0)
      };
    }));
  ...
  try {
    ...
    const classroomsWithCount = await Promise.all(classrooms.map(async (c) => {
      ...
      if (c.type === 'home' && c.students && c.students.length > 0) {
         try {
           const student = await User.findById(c.students[0]).select('homeCourseDetails name');
           if (student && student.homeCourseDetails && c.course) {
             const details = student.homeCourseDetails.find(d => d.course && d.course.toString() === c.course._id.toString());
              if (details) {
                 extraDetails = {
                   address: details.address,
      ...
      return {
        ...c,
        ...extraDetails,
        studentCount: (c.students && c.students.length > 0) ? c.students.length : (c.studentCount || 0)
      };
    }));
...
exports.enrollStudent = async (req, res) => {
  try {
    ...
    if (course.type === 'home') {
       // Logic for home courses
       // Find or create a "Home" classroom SPECIFIC for this student
       ...
       if (!availableClassroom) {
         ...
         availableClassroom = new Classroom({
           name: `Residência ${user.name}`,
           course: courseId,
           professor: course.instructor,
           professors: [], // Start with no extra professors
           capacity: 1, // Only this family/student
           type: 'home',
           description: 'Sala virtual para alunos de curso domiciliar',
           status: 'active'
         });
         await availableClassroom.save();
       }
 
       // Initialize home details for the user
       await User.findByIdAndUpdate(userId, {
        ...
        availableClassroom = classrooms.find(c => {
...
exports.getClassroomById = async (req, res) => {
  try {
    const classroom = await Classroom.findById(req.params.id)
      .populate('course', 'title')
      .populate('professor', 'name profileImage bio specialization')
      .populate('professors', 'name profileImage bio specialization')
      .lean();
    ...
    classroom.studentCount = (classroom.students && classroom.students.length > 0) ? classroom.students.length : (classroom.studentCount || 0);

    // Home course specific logic
    if (classroom.type === 'home') {
        const homeProfessorsCount = await User.countDocuments({ role: 'professor', professorType: 'home' });
        classroom.homeProfessorsCount = homeProfessorsCount;
    }
...
exports.createClassroom = async (req, res) => {
  const classroom = new Classroom({
    ...
    type: req.body.type,
    externalCourseName: req.body.externalCourseName,
...
exports.updateClassroom = async (req, res) => {
  try {
    ...
    if (req.body.professors !== undefined) classroom.professors = req.body.professors;
    ...
    if (req.body.externalCourseName !== undefined) classroom.externalCourseName = req.body.externalCourseName;
    if (req.body.budget !== undefined) classroom.budget = req.body.budget;
    if (req.body.duration !== undefined) classroom.duration = req.body.duration;
    if (req.body.guardianContact !== undefined) classroom.guardianContact = req.body.guardianContact;
    
    // External Details Fields
    if (req.body.studentCount !== undefined) classroom.studentCount = req.body.studentCount;
    if (req.body.ageRange !== undefined) classroom.ageRange = req.body.ageRange;
    if (req.body.address !== undefined) classroom.address = req.body.address;
    if (req.body.phoneNumber !== undefined) classroom.phoneNumber = req.body.phoneNumber;

    const updatedClassroom = await classroom.save();
    await updatedClassroom.populate('course', 'title');
    await updatedClassroom.populate('professor', 'name');
    await updatedClassroom.populate('professors', 'name');
